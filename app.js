...
function loadVideoFromInput() {
  ...
  els.transcriptStatus.textContent = currentVideoId && currentVideoId !== videoId
...
function setEmptyWorkspace() {
  ...
  els.promptText.textContent = "Tải video và nạp phụ đề có timestamp. Hệ thống sẽ tự tạo câu để nghe, nhập và kiểm tra.";
...
function parseTimestamp(timestamp) {
  ...
  if ([secondsPart, minutesPart, hoursPart].some(Number.isNaN)) return NaN;
...
function isNonSpeechCue(text) {
  ...
  const lettersOrNumbers = compact.replace(/[♪♫♬\s.,!?;:'"“”‘’()[\]{}…—-]/g, "");
...
function parseSubtitleContent(content, skipNonSpeech = true) {
  ...
  blocks.forEach((block) => {
    ...
    const [rawStart, rawEndAndSettings] = timingLine.split("-->").map((part) => part.trim());
...
function loadTranscriptFromText(content) {
  const { segments, skipped } = parseSubtitleContent(content, els.skipNonSpeechToggle.checked);
  if (!segments.length) {
    els.transcriptStatus.textContent = "Không đọc được câu thoại nào. Hãy kiểm tra file có đúng định dạng SRT/VTT và có timestamp hay không.";
  ...
  const skippedText = skipped > 0 ? ` • Đã bỏ qua ${skipped} đoạn trống / không lời thoại.` : "";
  els.transcriptStatus.textContent = `Đã nạp ${segments.length} câu có lời thoại.${skippedText}`;
  setAnswerStatus("✅ Script đã cập nhật theo video. Bạn có thể bắt đầu chép chính tả.", "success");
}
...
function clearTranscript() {
  ...
  resetLessonSession();
  els.subtitlePasteInput.value = "";
  els.transcriptStatus.textContent = "Đã xóa script hiện tại. Hãy nạp phụ đề mới để tiếp tục.";
  setEmptyWorkspace();
}
...
function updateSegmentUI() {
  ...
  els.promptText.textContent = "Nghe đoạn video và chép lại chính xác câu vừa phát. Nút Next chỉ mở khi câu đúng 100%.";
  els.nextBtn.disabled = true;
  els.recordBtn.disabled = true;
  els.answerInput.value = "";
  els.reviewLine.textContent = "Kết quả kiểm tra từng từ sẽ hiển thị tại đây.";
